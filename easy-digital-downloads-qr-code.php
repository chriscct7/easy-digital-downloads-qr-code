		...
		class QR_Code_Main {
			...
			function download_image() {
				...
				if ( !empty($_POST['security']) && wp_verify_nonce( $_POST['security'], 'download-qr-code' ) ) {
					...
					$purchase_link = edd_get_purchase_link( array( 'download_id' => $product_id ) );
			...
			function qrcode_inner_custom_box( $post ) {
				...
				<div id="qr-results">
					...
					<p><a id="generated-qr-code-large"><?php _e('Download large (1440x1440)', 'edd_qrcode'); ?></a><br/>
						<?php _e('(right click, save link as)', 'edd_qrcode'); ?>
					</p>
				...
				<p>
					<label class="radio">
						<input type="radio" name="edd_qrcodegen[selection]" id="add_to_cart_url" value="add_to_cart_url" <?php checked($meta['selection'], 'add_to_cart_url', true); ?>>
						<?php _e('Add to cart URL', 'edd_qrcode'); ?>
					</label>
				</p>

				<p>
					<label class="radio">
						<input type="radio" name="edd_qrcodegen[selection]" id="product_url" value="product_url" <?php checked($meta['selection'], 'product_url', true); ?>>
						<?php _e('Product\'s page', 'edd_qrcode'); ?>
					</label>
				</p>
				<?php
...
function edd_qr_license_page() {
	...
	?>
			<div class="wrap">
				<h2><?php _e('EDD QR License Options'); ?></h2>
				<form method="post" action="options.php">
				
				<?php settings_fields('edd_qr_license'); ?>
				...
				<table class="form-table">
					<tbody>
						<tr valign="top">	
							...
							<td>
								<input id="edd_qr_license_key" name="edd_qr_license_key" type="text" class="regular-text" value="<?php esc_attr_e( $license ); ?>" />
								<label class="description" for="edd_qr_license_key"><?php _e('Enter your license key'); ?></label>
							</td>
						</tr>
						...
						<tr valign="top">	
							...
							<td>
								<?php if( $status !== false && $status == 'valid' ) { ?>
									...
									<?php wp_nonce_field( 'edd_qr_nonce', 'edd_qr_nonce' ); ?>
		...
		function edd_qr_activate_license() {
			...
			// Call the custom API.
			$response = wp_remote_get( add_query_arg( $api_params, EDD_QR_STORE_URL ), array( 'timeout' => 15, 'sslverify' => false ) );
